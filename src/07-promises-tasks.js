...
function chainPromises(array, action) {
  return array.reduce((promiseChain, task) => promiseChain
    .then((chainResults) => task.then((currentResults) => [...chainResults, currentResults]))
    .catch((chainResults) => chainResults),
  Promise.resolve([]))
    .then((results) => {
      let result = results[0];
      for (let i = 1; i < results.length; i += 1) {
        result = action(result, results[i]);
      }
      return result;
    });
}
